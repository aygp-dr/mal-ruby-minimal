...
# Adds quote, quasiquote, unquote, and splice-unquote
...
def EVAL(ast, env)
  ...
  loop do
    # Macro expansion would go here in step 8
    ast = macroexpand(ast, env) if respond_to?(:macroexpand)
...
def create_core_fns
  ...
  # List manipulation
  %w[cons concat].each { |op| core[op] = op }
...
if __FILE__ == $0
  ...
  # Interactive REPL
  puts "Step 7: Quote and Quasiquote"
  puts "Try: (quote (+ 1 2))"
  puts "Or: `(list 1 ~(+ 2 3) 4)"
